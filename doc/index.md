...
## Event Lifecycle and Message Passing
...
New [[dora-hyoujihai]](s) are possibly revealed upon the following events:

- `deal`: always reveal one immediately
- `dahai`: reveal postponed
- `ankan`, `kakan`, `daiminkan`: reveal postponed; depending on rule variations, may reveal one immediately or postpone one

This revealing is implemented using an extra property `newDoraHyouji` in these events. (details TBD after write-up of client-server)

[[Ura-dora]](s) are revealed when `ron` or `tsumoAgari` is applied.
